...
module.exports = function(models, knex) {
    ...
    var LastSeenCommand = new Command(/^\!lastseen +(<@!?([0-9]+)>)$/i, function (client, user, userId, channelId, message, event) {
        ...
        knex('users').select('last_seen_at').where('discord_id', targetUserId).then(function(result) {
            if(result.length > 0) {
                var dateTime = moment(result[0].last_seen_at);
    ...
    /*
     * Return specified users XP, level and ranking position
     * Usage: .rank [@user]
     */
    var RankCommand = new Command(/^\!rank( +)?(<@!?([0-9]+)>)?/i, function (client, user, userId, channelId, message, event) {
        ...
        /*
         * Run query to get list of user_id and the their total XP
         */
        knex('messages').select('user_id').sum('xp as total').groupBy('user_id').orderBy('total', 'desc').then(function(rows) {
            var total = 0;
            var rank = rows.length + 1;

            /*
             * Loop through all rows until we find the target user to get their total XP and ranking position
             */
            for(var i in rows) {
                if(rows[i].user_id === targetUserId) {
                    total = rows[i].total;
                    rank = parseInt(i) + 1;
                }
            }
            ...
            client.sendMessage({
                to: channelId,
                message: util.format('<@!%s> XP: %s, Level: %d, Rank: %d',
                    targetUserId, formattedTotal, YuiLevels.levelForXp(total), rank)
            });
        });
    ...
    return {
        ...
        PokeBackCommand: PokeBackCommand,
